...
class Wishlist_Column{
	...
	public function add_new_columns( $columns ){
		...
		$new_column = array(
			'wishlist-item-count' => esc_html__( 'Total Items', 'woo-wishlist' ),
		);
	...
	public function new_columns_content( $column, $post_id ){
		...
		if( 'wishlist-item-count' == $column ) {
			...
			echo sprintf( "<center><i><strong>%d</strong> <span>%s</span></i></center>", $total_items, __('Items on this List', 'woo-wishlist') );
		}
	...
	public function add_display_post_states( $post_states, $post ){
		...
		if ( get_option( 'pickplugins_wl_wishlist_page' ) == $post->ID ) {
			...
			$post_states['wishlist_page'] = __( 'Wishlist page', 'woo-wishlist' );
		}
	...
	$actions['view'] = sprintf( "<a href='%s' rel='bookmark' aria-label='%s'>%s</a>", $single_url, get_the_title(), __('View', 'woo-wishlist') );
	 *
	 * */
