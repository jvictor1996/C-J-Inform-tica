...
function pickplugins_wl_get_single_wishlist_html( $wishlist_id = 0 ){
	...
	$status_hint_text	= $wishlist_status == 'private' ? __('Private List', 'woo-wishlist') : __('Public List', 'woo-wishlist');
    ...
    $html .= sprintf("<small>%s : %s</small>", __('Items', 'woo-wishlist'), $total_items );
	...
	$html .= sprintf("<span class='createdby hint--top' aria-label='%s'><i class='fa fa-user'></i> <span>%s</span></span>",
			__('Created by', 'woo-wishlist'), get_the_author_meta( 'display_name' ) );
